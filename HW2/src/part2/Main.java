...
public class Main {
    ...
    public void parseData(String path, part2.State state){
        ...
        for (CSVRecord record : seFileUtil.getCSVParser()) {
            ...
            } catch (IllegalOperationRequest | CustomerTypeIllegalException
        ...
        switch (customerType) {
            case "Regular" -> {
                ...
                regularCustomer.handleInfraStructureProviders(command[0],
                        state.getInfraStructureProvider(command[2]), state);
            }
            case "Government" -> {
                Customer governmentCustomer = new GovernmentCustomer(id);
